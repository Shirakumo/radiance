...
おそらく、あなたが一番したいことは、ユーザにHTMLを提供することでしょう。その方向に進めながら、少しずつ拡張していきましょう。まずは、Radianceの準備をしましょう。
...
Radianceを使うのが初めての場合は、`r-welcome`モジュールを使いながら進めましょう。`r-welcome`モジュールを使うと、ブラウザーで開く最初のページへリンクをはることができます。まずは、小さなページにリンクをはりましょう。
...
普通であれば、フォントの書式を変えたり、CSSファイルを追加したりしながら、スタイルを作っていきます。
CSSファイルをサーブすることは可能ですが、長い目でみれば、最適の方法とはいえません。

代わりに、モジュール(module)を作る方法を紹介します。
モジュールを使うことで、Webページを適切に整理することができます。
モジュール(module)に必要なファイルは手動で作れますが、ここでは雛形を用いてモジュールを自動生成しましょう。

```common-lisp
(create-module "example")
```

モジュールが自動生成されると、モジュールのパスを返します。
生成されるものは、ASDFのシステムファイル、lispのmainファイル、`static`と`template`のフォルダ２つです。
