...
\noindent If the connection fails, an error of type \inline{DATABASE-CONNECTION-FAILED} is signalled. If a connection is already open, the previous connection is closed automatically and a warning of type \inline{DATABASE-CONNECTION-ALREADY-OPEN} is signalled.
...
\noindent A warning of type \inline{DATABASE-CONNECTION-NOT-OPEN} is signalled if the connection was already closed.
...
\noindent If the database cannot store a value because it exceeds the range, an error of type \inline{DATABASE-INVALID-VALUE} is signalled. It is not guaranteed that this error occurs if a value exceeds the specified range since the implementation may have had to upgrade it.
...
\noindent If an attempt is made to store a string that exceeds the specified length, an error of type \inline{DATABASE-INVALID-VALUE} is signalled.
...
Creates a new collection in the database. If the implementation supports structure, it is defined according to the given \inline{structure} list. Every collection automatically adds an \inline{_id} field as specified above. \\

\noindent If the \inline{collection} name is not extended alphanumeric, an error of type \inline{DATABASE-INVALID-COLLECTION} is signalled. If an additional \inline{_id} field is declared in the \inline{structure} definition, the name of a field is not extended alphanumeric, or a field's type declaration is invalid or unsupported, an error of type \inline{DATABASE-INVALID-FIELD} is signalled. If \inline{if-exists} is set to \inline{:error} and the table exists already, an error of type \\\inline{DATABASE-COLLECTION-ALREADY-EXISTS} is signalled.
...
\subsection{Records}
A collection stores data in the form of records, each record being a map of fields to their values. Each record contains an \inline{_id} value that uniquely identifies the record within its collection.
...
\extfuncdef{db:iterate}{collection query function \&key fields skip amount sort accumulate}{list}{
  collection &---& A string naming the collection.\\
  query &---& A query object, usually generated by \inline{db:query}\\
...
\extfuncdef{db:insert}{collection data}{\inline{_id}}{
  collection &---& A string naming the collection.\\
  data &---& A hash-map or an alist of the data for the new record.\\
}
Inserts the \inline{data} as a new record into the \inline{collection}. Returns the value of the \inline{_id} field of the newly generated record. \\

\noindent If the \inline{data} contains an \inline{_id} field or an inexistent field, an error of type \inline{DATABASE-INVALID-FIELD} is signalled. If the collection does not exist, an error of type \inline{INEXISTENT-DATABASE-COLLECTION} is signalled. 
