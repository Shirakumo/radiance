...
(defmacro defimplclass (slot superclass)
  ...
  `(progn 
     (log:info :radiance.server.implementation "Defining implementation ~a with ~a" ',slot ',superclass)
     ...
     (defmethod implement ((slot (eql ',slot)) (module ,superclass))
       (log:info :radiance.server.implementation "~a implements ~a" module slot)
...
(defmacro defimpl (slot &body generics)
  ...
  (let ((documentation "") (mod-gens (gensym "IMPL-GENSYM"))
    ...
    (log:info :radiance.server.implementation "Generating implementation ~a with superclasses ~a." slot super)
    `(progn
       (defclass ,slot ,super ()
...
(defun load-implementations (&key force)
  (loop for (key . val) in (config :implementations)
     do (progn (log:info :radiance.server.implementation "Choosing ~a for ~a." val key)
               (if (listp val)
