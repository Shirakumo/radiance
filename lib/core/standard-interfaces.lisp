...
also invoke AUTH:AUTHENTICATE."))
  ...
  (api-call (apicall method &key post get)
    (:documentation "Executes an API call by method, simulating the specified post and get vars. The expected format is an ALIST for both."))
  (dispatch (request)
...
(define-interface (database db)
  ...
  (count (collection query)
    (:documentation "Returns the count of result rows the query would return."))
