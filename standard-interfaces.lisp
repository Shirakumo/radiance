...
(define-interface (database db)
  ...
  (define-condition invalid-collection (error collection-condition)
    ()
    (:report (lambda (c s) (format s "The collection ~s does not exist." (slot-value c 'database::collection)))))
  (define-condition collection-already-exists (error collection-condition)
    ()
    (:report (lambda (c s) (format s "The collection ~s already exists." (slot-value c 'database::collection)))))
  (define-condition invalid-field (error condition)
    ((field :initarg :field :initform (error "FIELD required.")))
    (:report (lambda (c s) (format s "The field ~s does not exist." (slot-value c 'database::field)))))
  (deftype id ())
...
(define-interface user
  ...
  (define-condition not-found (error condition)
    ...
    (:report (lambda (c s) (format s "The user ~s does not exist." (slot-value c 'user::name)))))
  (defclass user () ())
  (defun = (user-a user-b))
  (defun list ())
