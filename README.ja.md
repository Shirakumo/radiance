...
普通であれば、ここから、フォントの書式を変えたり、CSSを追加しながら、ページのデザインを作っていくかと思います。確かにHTMLにCSSを読み込ませることも可能ですが、長い目でみれば最適の方法とはいえません。
...
ページは、シンボルの名前で特定化します。独自にモジュールを作成すると、新しいパッケージが生成されます。先例でのシンボル(example)は、一度も使われていないシンボルです。パッケージ内で名前衝突を避けるために、`rad-user`パッケージでページを消す必要があるかもしれません。
...
RadianceのURIは、URIから一般的な要素を抽出したもので、スキーマ、クエリ、フラグメント等は含みません。また、`domains`のURIは、フレームワークにおいて複数の場所で使われるのが通常と異なる点です。例えば、`location`を捕捉するときや、`dispatch matching`を処理するために使われます。

URIは変更可能です。ただし、URIの修正はクリティカルパスがある場所で行われるので、パフォーマンスに重大な影響を与えます。また、想定外の方法でURIを修正した場合、予期しない動作が発生する可能性があります。
